$(document).ready(function() {
    $('#yp_manager_jh_process_leftpage').load("public/pub/code/index.jsp?c=goods_type&n=商品类型&t=pub",
    function() {
            ...
            onClick: function(node) {
                ...
                if (gym_tree == null) {
                    yp_goods___yp_goodsparams = {
                        sql: "select * from yp_manager_cust where good_type=? and gym=?",
                        sqlPs: [node.id, gym]
                    };
                } else {
                ...
                cq('yp_manager___yp_goods', 'task', yp_goods___yp_goodsparams);
            }
        });
    });
    $('#yp_manager_jh_process_leftpage1').load("public/pub/areaCode/index.jsp?c=area_code&n=行政区域&t=pub",
    function() {
        $('#gym_tree').tree({
            onClick: function(node) {
                ...
                if (code_tree == null) {
                    yp_goods___yp_goodsparams = {
                        sql: "select * from yp_manager_cust where gym=?",
                        sqlPs: [node.id]
                    };
                } else {
                    var type = code_tree.id;
                    yp_goods___yp_goodsparams = {
                        sql: "select * from yp_manager_cust where gym=? and good_type=?",
                        sqlPs: [node.id, type]
                    };
                }
