...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    var url;
    var keys=res.key;
    var names=keys.split('_._');
    var filename=names[names.length-1];
    if (res.url) {
        url = res.url;
        str = "<div><strong>下载地址:</strong><a name='uploaded_file' data-name='"+filename+"' href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
        ...
        str = "<div><strong>下载地址:</strong><a name='uploaded_file' data-name='"+filename+"' href=" + url + " target='_blank' > " + link + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
    ...
    var showImg = $('<img src="public/sb_admin2/bower_components/qiniu/images/loading.gif"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', 'public/sb_admin2/bower_components/qiniu/images/default.png');
        ...
        $(img).on('load', function() {
            ...
            function initImg(url, key, height) {
                ...
                modalBody.find('img').attr('src', '/public/sb_admin2/bower_components/qiniu/images/loading.gif');
