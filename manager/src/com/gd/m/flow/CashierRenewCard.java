 ...
 *
 */
public class CashierRenewCard {
	private SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd ");
	Date deadline = null;
	String  cardName="";

	public String xx(JSONObject con, Connection conn, String cust_name, String gym, String memId, String caAmt,
			String trealAmt, String flownum) throws Exception {
		String recordId = "";
		String type_code = con.getString("type_code");
		String typ_user_id = con.getString("typ_user_id");
		String userGym = con.getString("userGym");
		...
		String type_name = "";
		if (size > 0) {
			cardName=en.getStringValue("type_name");
			...
			if ("001".equals(card_type)) {
				...
				String sql = "select a.state,a.id,a.deadline,a.times from yp_type_user_" + gym
						+ " a,yp_type b where a.type_code = b.type_code and b.card_type='001' and b.cust_name=?  and a.id=?";
				size = en.executeQuery(sql, new String[] { cust_name, typ_user_id });
				if (size > 0) {
				...
				// 判断是否已经有购买了的储值卡
				if (size > 0) {
					String type_user_id = en.getStringValue("id");
					// 已经有购买了的储值卡了
					Date deadline = en.getDateValue("deadline");
					int oldTimes = en.getIntegerValue("times");
					if (deadline == null) {
					...
					en = new EntityImpl(conn);
					sql = "update yp_type_user_" + gym + " set times=?, deadline=?, type_code=? where id=?";
					en.executeUpdate(sql,
							new String[] { oldTimes + times + "", sdf.format(newDate), type_code, type_user_id });
				...
				Date newDate = cld.getTime();
				deadline = newDate;
				recordId = createBuyCardsRecord(en, gym, cust_name, type_code, "admin", memId, "001", "", caAmt, conn,
						flownum);
				buy(en, userGym, cust_name, type_code, memId, newDate, times, mins, "001", "admin", "", recordId, conn);
				d = newDate;
			}
		}
		...
		body.put("deadline", Utils.parseData(deadline, "yyyy-MM-dd HH:mm"));
		body.put("pay_time", Utils.parseData(new Date(), "yyyy-MM-dd HH:mm"));
		body.put("name", cardName);
		body.put("price", Utils.toPrice(trealAmt));
		//MsgToMemAction.SendMsg("mem_add_block", memId, body, conn, cust_name, gym);
		return recordId;
	}
